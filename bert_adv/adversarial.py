...
def _mask_by_length(t, input_mask):
  """Mask t, 2-D [batch, dim], by length, 1-D [batch,]."""
  max_len = t.shape[2]
  mask = tf.tile(tf.expand_dims(input_mask, axis=-1), [1, 1, max_len])
  masked = tf.multiply(t,  tf.cast(mask, tf.float32), name=t.name[:-2])
  return masked

def random_perturbation(embedded, input_mask):
  """Adds noise to embeddings and recomputes classification loss."""
  noise = tf.random_normal(shape=tf.shape(embedded))
  noise = _mask_by_length(noise, input_mask)
  perturb = _scale_l2(noise, FLAGS.perturb_norm_length)
  return perturb
...
def _scale_l2(x, norm_length, name=None):
  ...
  x_unit = tf.divide(x, l2_norm)
  out = tf.multiply(norm_length, x_unit, name=name)
  return out
